...
load("//platformio/private:defs.bzl", "PlatformIOLibraryInfo", "COPY_COMMAND", "SHELL_HEADER")
...
def _emit_build_action(ctx, project_dir, output_files):
    ...
    commands.append(_BUILD_COMMAND.format(platformio = ctx.executable.tool.path, project_dir = project_dir))
...
def _emit_executable_action(ctx, project_dir):
    ...
    commands.append(_UPLOAD_COMMAND.format(platformio = ctx.executable.tool.path, project_dir = project_dir))
    ctx.actions.write(
